...
const links = [["Артықшылықтарымыз", "#features"], ["Оқушылар нәтижесі", "#results"], ["Платформа", "#platform"], ["Тарифтер", "#pricing"]];
...
export default function Header() {
  ...
  return <motion.header initial={{ y: -90, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: .65 }} className="fixed inset-x-0 top-5 z-50 px-3">
    <div className="mx-auto flex h-[82px] max-w-[1180px] items-center rounded-[2rem] bg-white px-6 shadow-[0_4px_50px_rgba(0,0,0,.07)] sm:px-8">
      ...
      <div className="ml-auto hidden items-center gap-3 sm:flex"><a href="#platform" className="rounded-[15px] border border-sky-300 px-4 py-3 text-xs font-bold text-sky-600">LMS-ке кіру</a><a href="#contact" className="rounded-[15px] bg-[#2eb9eb] px-5 py-3 text-xs font-bold text-white transition hover:bg-[#1e7de5]">Курсқа қосылу</a></div>
