...
export default function TrainingPath() {
  return <section id="program" className="section-space bg-white"><div className="site-container">
    ...
    <div className="relative grid gap-5 lg:grid-cols-4">{steps.map(({Icon,number,title,text},index)=><FadeIn key={title} delay={index*.07}>
      <motion.article whileHover={{y:-8}} className="relative h-full min-h-[250px] overflow-hidden rounded-[1.6rem] border border-slate-200/80 bg-[#f8fafc] p-6 shadow-[0_4px_60px_rgba(0,0,0,.045)] transition-shadow hover:shadow-xl sm:min-h-[310px] sm:rounded-[2rem] sm:p-7">
