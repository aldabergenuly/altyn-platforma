...
function ProgramCard({ program, index }: { program: Program; index: number }) {
  ...
  return <motion.article initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, delay: index * .06, ease }} className={`flex min-h-[330px] flex-col rounded-[1.6rem] border p-5 sm:min-h-[390px] sm:rounded-[1.75rem] sm:p-7 ${locked ? "border-slate-200 bg-slate-50/80" : soon ? "border-amber-200 bg-white shadow-[0_18px_55px_rgba(245,158,11,.08)]" : "border-sky-200 bg-white shadow-[0_22px_65px_rgba(46,185,235,.12)]"}`}>
...
export default function Pricing() {
  ...
  return <section id="pricing" className="section-space bg-white"><div className="site-container">
    ...
    <div role="tablist" aria-label="Олимпиада пәндері" className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
      {subjects.map(subject => { const Icon = subject.icon; const isActive = selected === subject.id; return <motion.button whileHover={{ y: -4 }} key={subject.id} role="tab" aria-selected={isActive} aria-controls="subject-programs" onClick={() => setSelected(subject.id)} className={`group flex min-h-[132px] flex-col items-start justify-between rounded-[1.35rem] border p-4 text-left transition-all duration-300 sm:min-h-40 sm:rounded-[1.6rem] sm:p-5 ${isActive ? "border-blue-600 bg-gradient-to-br from-[#2eb9eb] to-[#2563eb] text-white shadow-[0_20px_55px_rgba(37,99,235,.24)]" : "border-slate-200 bg-[#f8fafc] text-slate-900 hover:border-sky-300 hover:bg-white hover:shadow-xl"}`}>
