...
export default function Features() {
  return <section id="features" className="section-space bg-[#f1f2f4]"><div className="site-container">
    ...
    <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{features.map(([Icon,title,copy,theme],i) => {
      ...
      return <FadeIn key={title} delay={i*.05}><motion.article whileHover={{y:-8}} className={`group relative min-h-[255px] overflow-hidden rounded-[1.6rem] p-6 shadow-[0_4px_80px_rgba(0,0,0,.06)] sm:min-h-[320px] sm:rounded-[2rem] sm:p-8 ${dark?"bg-[#05090e] text-white":blue?"bg-[#1e7de5] text-white":"bg-white text-[#1d1d1d]"}`}>
