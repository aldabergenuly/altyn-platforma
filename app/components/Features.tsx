...
import { Activity, BrainCircuit, Clock3, FlaskConical, Medal, MessagesSquare } from "lucide-react";
import { FadeIn, SectionHeading } from "./shared";

const features = [[FlaskConical, "Олимпиадалық сынақ", "Апта сайын нақты олимпиада форматындағы сынақ пен толық талдау."], [Activity, "Жеке прогресс", "Әлсіз тақырыптарды көріп, өсу динамикасын нақты метрикамен бақыла."], [MessagesSquare, "Жеке ментор", "Жоспар, мотивация және күрделі есептер бойынша тұрақты қолдау."], [BrainCircuit, "Терең пәндік база", "Химияны формула емес, өзара байланысты жүйе ретінде түсіну."], [Medal, "Жеңімпаздар ортасы", "Мақсаты биік оқушылармен бірге дамып, жоғары стандарт қалыптастыру."], [Clock3, "24/7 қолжетімділік", "Сабақ пен материалдар кез келген уақытта барлық құрылғыда ашық."]];
