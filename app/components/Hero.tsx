...
const coaches: Coach[] = [
  { name:"Тренер 01", subject:"Математика", image:"/coaches-v1.jpg", imagePosition:"0% 32%", sprite:true, tilt:"hero-tilt-left", focus:"Олимпиадалық есептердің стратегиясы", skills:["Күрделі есепті кезеңдерге бөлу", "Дәлелдеу мәдениетін қалыптастыру", "Республикалық кезең форматын талдау"] },
  { name:"Алишер Қазтаев", subject:"Информатика", image:"/alisher-qaztayev.jpg", imagePosition:"center 18%", tilt:"hero-tilt-right", focus:"Олимпиадалық программалау және алгоритмдер", skills:["Алгоритмдік ойлау және есепті модельдеу", "Деректер құрылымдары мен тиімді шешімдер", "Олимпиада форматында код жазу және талдау"] },
  { name:"Ақберді Алиби", subject:"Физика", image:"/akberdi-alibi.jpg", imagePosition:"center 16%", tilt:"hero-tilt-left", focus:"Физикалық модель құру және күрделі есептер", skills:["Механика есептерінің негізгі әдістері", "Электродинамикадағы байланыстарды түсіну", "Олимпиадалық есепті жүйелі талдау"] },
  { name:"Тренер 04", subject:"Биология", image:"/coaches-v1.jpg", imagePosition:"100% 32%", sprite:true, tilt:"hero-tilt-right", focus:"Жүйелік ойлау және олимпиада практикасы", skills:["Тақырыптарды байланыстыру", "Зертханалық турға дайындық", "Терминологияны дәл қолдану"] },
];

function portraitStyle(coach: Coach) {
  return {
    backgroundImage: `url(${coach.image})`,
    backgroundPosition: coach.imagePosition,
    backgroundSize: coach.sprite ? "400% auto" : "cover",
  };
}
  ...
  return <section id="coaches" className="relative overflow-hidden bg-[#f1f2f4] pb-24 pt-28 sm:pt-32">
    <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.75,ease}} className="hero-marquee-shell">
      <div className="hero-marquee-track">
        {loop.map((item,index) => <button key={`${item.name}-${index}`} type="button" onClick={()=>setActive(index%coaches.length)} className={`hero-coach-card ${item.tilt}`} style={portraitStyle(item)} aria-label={`${item.name}, ${item.subject} тренері туралы ашу`}>
          <span className="coach-card-label"><span><b>{item.name}</b><small>{item.subject}</small></span></span>
        </button>)}
      </div>
    ...
    <AnimatePresence>{coach && <motion.div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/65 p-4 backdrop-blur-sm" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={(event)=>event.currentTarget===event.target&&setActive(null)} role="presentation">
      <motion.div initial={{opacity:0,y:25,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:20,scale:.96}} transition={{duration:.3,ease}} className="relative grid w-full max-w-4xl overflow-hidden rounded-[2rem] bg-white shadow-2xl md:grid-cols-[.8fr_1.2fr]" role="dialog" aria-modal="true" aria-label={`${coach.subject} тренерінің профилі`}>
        <div className="coach-modal-portrait min-h-[330px] md:min-h-[560px]" style={portraitStyle(coach)}/>
