...
export default function StatsBar() {
  return <section id="intensive" className="bg-[#f1f2f4] pb-28"><div className="site-container">
    <div className="mx-auto mb-10 max-w-4xl text-center sm:mb-12"><p className="text-xs font-semibold text-[#2eb9eb] sm:text-sm">ALTYN SYSTEM</p><h2 className="mt-3 text-[2rem] font-bold leading-[1.18] tracking-[-.025em] sm:text-5xl">ALTYN School интенсиві — нақты сандармен</h2><p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-slate-500 sm:mt-5 sm:leading-7">Теория, практика, апта сайынғы бақылау және менторлық қолдау бір 8 апталық ритмге біріктірілген.</p></div>
    <div className="grid gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-6">{intensive.map(({value,title,text,Icon,theme},index)=><motion.article key={title} initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:index*.06}} whileHover={{y:-8}} className={`relative min-h-[255px] overflow-hidden rounded-[1.6rem] p-6 shadow-[0_4px_80px_rgba(0,0,0,.05)] sm:min-h-[320px] sm:rounded-[2rem] sm:p-7 ${index<3?"lg:col-span-2":"lg:col-span-3"} ${theme}`}>
