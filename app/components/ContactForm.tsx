...
export default function ContactForm() {
  ...
  return <section id="contact" className="section-space bg-[#f1f2f4]"><div className="site-container">
    <motion.div initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="relative grid min-h-[650px] overflow-hidden rounded-[2rem] border border-slate-200 bg-[#f3f5f8] shadow-[0_4px_100px_rgba(0,0,0,.06)] lg:grid-cols-[.9fr_1.1fr]">
      ...
      <div className="coach-contact-portrait relative min-h-[500px] overflow-hidden lg:min-h-0" role="img" aria-label="ALTYN School олимпиада тренері">
        <div className="absolute bottom-7 right-7 rounded-2xl bg-slate-900/85 px-5 py-4 text-white shadow-xl backdrop-blur"><b className="block text-lg">ALTYN тренері</b><small className="mt-1 block text-slate-300">Диагностика · Жеке бағыт</small></div>
      </div>
    </motion.div>
  </div></section>;
}
